...
const DialogContent = ({ className, children, forceMount, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & { forceMount?: boolean }) => {
  const content = (
    ...
    >
      {children}
      <DialogPrimitive.Close className="ring-offset-background focus:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute top-2 right-4 sm:top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none">
        <X className="h-4 w-4" />
