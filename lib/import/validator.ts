 ...
 */
import { z } from 'zod'
import type { MappedRow, RowValidationError, ValidationResult } from './types'
...
export const socialMediaSchema = z.object({
  ...
  url: z.url('Invalid social media URL').max(500, 'URL is too long'),
})
  ...
  if (!result.success) {
    result.error.issues.forEach((error) => {
      errors.push({
        ...
        value:
          error.path.length > 0
            ? String(row[error.path[0] as keyof MappedRow])
            : undefined,
      })
    })
  }
